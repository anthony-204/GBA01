...
\begin{itemize}[noitemsep]
  \item \textbf{Phase 1:} Library mode, core cable/fuse checks, Excel data migration, 12 unit tests.
  \item \textbf{Version 2 (current):} Completeness gate, manual entry, GBA-0002 output panel, fuse-protects-cable check, 26 unit tests.
  \item \textbf{Version 3 (planned):} Parallel fuses, admin UI, PDF export, installation derating.
\end{itemize}
...
where:
\begin{itemize}[noitemsep]
  \item $K$ = copper constant (A$\cdot\sqrt{\mathrm{s}}$/mm$^2$) from \texttt{copper-k-factors.json} by cable type;
...
\textbf{Pass/warning:} $\Delta V\% \leq$ voltage drop limit (default 3\%). Exceeding the limit yields \texttt{warning} in Version~2.
...
\begin{itemize}[noitemsep]
  \item \texttt{Calculator.tsx} --- mode tabs, library picker, triggers \texttt{calculate()}
  \item \texttt{ManualEntryForm.tsx} --- validated manual inputs
  \item \texttt{PdfResultsPanel.tsx} --- GBA-0002 required outputs
  \item \texttt{IncompleteVehiclePanel.tsx} --- blocked vehicle messaging
  \item \texttt{CheckCard.tsx} --- expandable check details
  \item \texttt{lib/db.ts} --- loads \texttt{bundle.json} into engine
\end{itemize}
...
\begin{longtable}{@{}p{5.5cm}ccp{4cm}@{}}
  \toprule
  \textbf{Requirement} & \textbf{V2} & \textbf{V3+} & \textbf{Notes} \\
  \midrule
  \endhead
  Machine library selection & \checkmark & & 37 vehicles \\
  Manual entry mode & \checkmark & & Validated inputs \\
...
\begin{quote}
\small
We have delivered \textbf{Version~2} of the Fuse and Cable Protection web application per specification GBA-0002. The tool analyses 24\,V machine starting circuits---battery, cable, starter, and fuse---using your fleet database and manufacturer fuse/cable libraries. It provides pass/fail engineering checks, suggested fuse ratings with part numbers, and a standardized cable/fuse output summary. The application runs in a standard web browser, is mobile-friendly, and separates calculation logic from the user interface for maintainability and testing. \textbf{26 automated tests} verify core calculations. The tool is a \textbf{design aid}; final selections require qualified engineering review before installation.
\end{quote}
